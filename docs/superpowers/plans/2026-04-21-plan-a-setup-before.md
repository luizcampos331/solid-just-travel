...
> Nota: a versão inicial do plano tinha uma única fixture `client` com `raise_server_exceptions=False` global, o que mascararia exceções inesperadas em outros testes. Separamos em `client` (estrita) para o caso normal e `client_swallow_500` (permissiva) usada apenas pelo teste do LSP trap (Task 16). A rejeição intencional de um loop reflexivo no `_patch_session_local` está documentada na docstring — os 3 monkeypatches explícitos são a evidência pedagógica do DIP violation, e substituir por magia enfraqueceria a lição.
